...
// Initial mock regulatory references data
...
// LocalStorage key
const STORAGE_KEY = 'gmp-gdp-regulatory-references';
...
export const RegulatoryProvider = ({ children }) => {
  ...
  const [references, setReferences] = useState(() => {
    const savedReferences = loadFromStorage(STORAGE_KEY, null);
    if (!savedReferences) {
      // First time - initialize with demo data
      saveToStorage(STORAGE_KEY, initialReferences);
  ...
  // Update reference
  const updateReference = (referenceId, updates) => {
    ...
    setReferences(prev => prev.map(ref => 
      ref.referenceId === referenceId ? updatedReference : ref
    ));
